...
while getopts "c:n:s" c; do
	...
	;;
    *)
	echo 'Usage: stress-subvol.sh [-options] MOUNTPOINT'
...
shift $(($OPTIND-1))
if [ $# -ne 1 ]; then
    echo 'For usage: stress-subvol.sh -?'
    exit 1
fi
...
while [ $p -le $nconcurrent ]; do
	...
	while true; do
	    ...
	    # subvol creation is slow, do not create for every dir
	    find $content -type d -mindepth 1 -exec mkdir-or-subvol $mountpoint/stress/$p/'{}' \;
